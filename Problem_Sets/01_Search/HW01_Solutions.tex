	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\item The \textbf{branching factor} here is 3, since it need to necessarily be the case that all tasks are assigned to processors, then at each step we determine whether we should assign the current task to a processor or not assign it at all.  Then in general, the branching factor is the number of processors + 1.
			...
			\item State space generated by depth-first search:
