...
class Classifier(object):
    ...
    def learn(self):
        ...
        self.centroids = [None for i in xrange(self.num_categories)]
        denominators = [0.0 for i in xrange(self.num_categories)]
        for row in self.training_set:
            category = row[3]
            if self.centroids[category] == None:
                self.centroids[category] = row[:3]
            else:
                self.centroids[category] = [self.centroids[category][i] + row[i] for i in xrange(3)]
            denominators[category] += 1.0

        for index, centroid in enumerate(self.centroids):
            self.centroids[index] = [self.centroids[index][j] / denominators[index] for j in xrange(3)]
